...
bash %{SOURCE0} --extract=$RPM_BUILD_ROOT%{INSTALL_DIR}
mv -f $RPM_BUILD_ROOT%{INSTALL_DIR}/cuda-toolkit/* $RPM_BUILD_ROOT%{INSTALL_DIR}
rm -r $RPM_BUILD_ROOT%{INSTALL_DIR}/cuda-toolkit
mv -f $RPM_BUILD_ROOT%{INSTALL_DIR}/cuda-samples $RPM_BUILD_ROOT%{INSTALL_DIR}/samples
#mv -f $RPM_BUILD_ROOT%{INSTALL_DIR}/cublas/* $RPM_BUILD_ROOT%{INSTALL_DIR}

#rm -r $RPM_BUILD_ROOT%{INSTALL_DIR}/cublas
rm $RPM_BUILD_ROOT%{INSTALL_DIR}/NVIDIA-Linux-x86_64-*.run
cd $RPM_BUILD_ROOT%{INSTALL_DIR}/lib64
ln -s stubs/libcuda.so .
...
find $RPM_BUILD_ROOT%{INSTALL_DIR} -name Makefile | xargs sed -i -e s,$RPM_BUILD_ROOT,,g
