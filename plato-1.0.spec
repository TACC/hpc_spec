...
# John Cazes and Joe Garcia have taken over this module
# 2019-06-12
#
...
-DHDF5_INCLUDE_DIRS:PATH=${TACC_HDF5_INC} \
-DHDF5_LIBRARY_DIRS:PATH=${TACC_HDF5_LIB} \
-DHDF5_LIBRARY_NAMES:STRING="hdf5_hl;hdf5;dl;z;sz" \
...
-DSEACASExodus_PREFIX:PATH=$TRILINOS_INSTALL_TEMP \
...
#Have to copy libPlatoApp.so by hand
cp -a $PD/build/platoengine/apps/services/libPlatoApp.so $RPM_BUILD_ROOT/%{INSTALL_DIR}/platoengine/lib/

###############################################################################
# For platoAnalyze ($PLATO_ROOT/src/platoanalyze)
# This configuration file is needed to build Plato Analyze 
#   and enable optimization through the Plato Engine
###############################################################################

cd $PD/src
#Add Experimental to the KokkosBatched namespace in BLAS3.hpp
sed -i -e's/namespace KokkosBatched/namespace KokkosBatched::Experimental/g' platoanalyze/src/BLAS3.hpp
#Add Experimental to the KokkosBatched namespace in PlatoMathHelpersTest.cpp
sed -i -e's/namespace KokkosBatched/namespace KokkosBatched::Experimental/g' platoanalyze/unit_tests/PlatoMathHelpersTest.cpp
#Add Teuchos_ParameterList include to PressureGradientProjectionResidual.hpp
sed -i '/#include <memory>/ a #include <Teuchos_ParameterList.hpp>' platoanalyze/src/PressureGradientProjectionResidual.hpp

mkdir -p $PD/{build,install}/platoanalyze && cd $PD/build/platoanalyze
ANALYZE_SRC=$PLATO_ROOT/src/platoanalyze

PLATO_ANALYZE_SRC=$PLATO_ROOT/src/platoanalyze
PLATO_ANALYZE_INSTALL=%{INSTALL_DIR}/platoanalyze
...
echo "XXXXXX  configing platoanalyze "
$CMAKE $ANALYZE_SRC \
-DCMAKE_INSTALL_PREFIX:PATH=$PLATO_ANALYZE_INSTALL \
...
-DPLATOENGINE_PREFIX:PATH=$PLATO_ENGINE_INSTALL_TEMP \
-DPLATOANALYZE_ENABLE_MPMD=ON \
-DAMGX_PREFIX:PATH=$AMGX_INSTALL_TEMP \
2>&1 | tee $PD/build/platoanalyze/.user.config.log

echo "XXXXXX  building platoanalyze "
#make -j $NPMAKE 2>&1 | tee $PD/build/analyze/.user.make.log
make -j $NPMAKE VERBOSE=1 2>&1 | tee $PD/build/platoanalyze/.user.make.log
echo "XXXXXX  installing platoanalyze "
make -j $NPMAKE DESTDIR=$RPM_BUILD_ROOT install 2>&1 | tee $PD/build/platoanalyze/.user.install.log

#Works for all the other installs, I will leave it for this one.
#mkdir -p $RPM_BUILD_ROOT/%{INSTALL_DIR}/platoanalyze
#cp -a $PD/install/platoanalyze/bin $RPM_BUILD_ROOT/%{INSTALL_DIR}/platoanalyze
#cp -a $PD/install/platoanalyze/lib $RPM_BUILD_ROOT/%{INSTALL_DIR}/platoanalyze
...
local analyze_dir		     = pathJoin(base_dir, "platoanalyze")
