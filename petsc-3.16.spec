...
if [ -z "${ext}" ] ; then
  export architecture=milan
else
  export architecture=milan-${ext}
fi
...
export XOPTFLAGS="-mavx2 -O2 -g"
case "${ext}" in 
  ( *complex* )
  export XOPTFLAGS="-mavx2 -O1 -g"
...
# Sundials
#
export SUNDIALS_OPTIONS="--with-sundials2=1 --download-sundials2"
...
export EXTERNAL_PACKAGES_DIR=/admin/build/admin/rpms/ls6/SOURCES/petsc-packages
...
rm -rf $RPM_BUILD_ROOT/%{INSTALL_DIR}/usr/lib/.build-id
...
%files %{PACKAGE}-debug
  ...
  %{INSTALL_DIR}/src 
  %{INSTALL_DIR}/milan-debug
  %{INSTALL_DIR}/milan-unidebug
  %{INSTALL_DIR}/milan-complexi64debug
  %{INSTALL_DIR}/milan-complexdebug
  %{INSTALL_DIR}/milan-i64debug
  %{INSTALL_DIR}/milan-single-debug
%if "%{cuda}" == "1"
  %{INSTALL_DIR}/milan-rtxdebug
%endif

%files %{PACKAGE}
  %defattr(-,root,install,)
  %{INSTALL_DIR}/milan
  %{INSTALL_DIR}/milan-uni
  %{INSTALL_DIR}/milan-complexi64
  %{INSTALL_DIR}/milan-complex
  %{INSTALL_DIR}/milan-i64
  %{INSTALL_DIR}/milan-single
%if "%{cuda}" == "1"
  %{INSTALL_DIR}/milan-rtx
%endif
  %{INSTALL_DIR}/milan-nohdf5
  %{INSTALL_DIR}/milan-hyprefei
...
%changelog
* Wed Oct 13 2021 eijkhout <eijkhout@tacc.utexas.edu>
