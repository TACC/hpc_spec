...
%define underscore_version 2_3_3
...
MVAPICH2-GDR 2.3.3 binary release is based on MVAPICH2 2.3.3 and incorporates designs that take advantage of the new GPUDirect RDMA technology, which enables direct P2P communication between NVIDIA GPUs and Mellanox InfiniBand adapters. MVAPICH2-GDR 2.3.3 offers significant improvements in latency and bandwidth for GPU-buffer based intranode and internode MPI Communication involving small and medium message sizes. For more information on the GPUDirect RDMA technology, refer to http://www.mellanox.com/page/products_dyn?product_family=116.

MVAPICH2-GDR 2.3.3 provides an efficient support for Non-Blocking Collectives (NBC) from GPU buffers to achieve maximal overlap. It uses novel designs that combine GPUDirect RDMA and Core-Direct technologies. Further MVAPICH2-GDR 2.3.3 also provides support for CUDA Managed memory features and optimizes large message collectives targeting Deep Learning frameworks.
...
This module loads the MVAPICH2-GDR MPI environment built with IBM XLC compilers.
...
local base_dir = "/opt/apps/xl16/mvapich2-gdr/2.3.3"
...
setenv( "LD_PRELOAD"             ,   "/opt/apps/xl16/mvapich2-gdr/2.3.3/lib64/libmpi.so" )
