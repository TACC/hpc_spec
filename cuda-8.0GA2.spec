...
Source0:    %{pkg_base_name}_%{pkg_version}.%{patch_version}_%{driver_version}_linux.run
Source1:    %{pkg_base_name}_%{pkg_version}.%{patch_version}.%{local_release}_linux.run
...
%global patch_options --silent --installdir=$RPM_BUILD_ROOT%{INSTALL_DIR} --accept-eula
bash %{SOURCE1} %{patch_options}
...
sed -i -e s,$RPM_BUILD_ROOT,,g $RPM_BUILD_ROOT%{INSTALL_DIR}/bin/.patch_8.0.61.2_uninstall_manifest_do_not_delete.txt
...
sed -i -e s,%{INSTALL_PREFIX},${POST_INSTALL_PREFIX},g ${POST_INSTALL_PREFIX}/%{INSTALL_SUFFIX}/bin/.patch_8.0.61.2_uninstall_manifest_do_not_delete.txt
