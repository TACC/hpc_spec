#
# Amit Ruhela
# 2020-05-15 Add name-defines-noreloc.inc
# 2020-05-15 Need to investigate relocation -- use /opt/apps for now
#
...
#cp /root/rpmbuild/SOURCES/pgi/pgilinux-2019-1910-ppc64le.tar.gz pgi-${pgi_version}.tar.gz
#tar -xfz pgi-${pgi_version}.tar.gz
cd /root/rpmbuild/SOURCES/pgi
...
cd  %{INSTALL_DIR}/linuxpower
rm -rf %{year}
rm -rf %{INSTALL_DIR}/modulefiles
cd  %{INSTALL_DIR}/..
...
appropriate to build and run on any compute node (login node, KNL compute
...
prepend_path( "LM_LICENSE_FILE"          , pathJoin(pgi_dir,"../../license.pgi"   )  )
...
prepend_path( "INCLUDE"                  , pathJoin(pgi_dir,"include_man"   )               )
prepend_path( "INCLUDE"                  , pathJoin(pgi_dir,"include-gcc70"   )               )
prepend_path( "MODULEPATH"               , "%{MODULE_PREFIX}/%{moduleversion}/modulefiles" )
...
%{PKG_BASE}/%{lib_dir}/bin/makelocalrc -x %{PKG_BASE}/%{lib_dir}/bin
