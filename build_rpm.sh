...
TEMP=`getopt -o hb:fdlg:x:i:j:p:m:s:o:v --long help,build:,debug,log,gcc:,xl:,intel:,pgi:,impi:,,mvapich2:,openmpi:,cmpich:,spectrum:,force,show,version \
     -n 'build' -- "$@"`
if [ $? != 0 ] ; then
...
while true ; do
  case "$1" in
    ...
    -s|--spectrum) mpi="spectrum"; spectrumV="$2"; shift 2 ;;
...
if [ -n "$help" ]; then
  ...
  echo "  -xV | --xl=V       : build with xl          with version V"
  ...
  echo "  -sV | --spectrum=V : build with spectrum    with version V"
