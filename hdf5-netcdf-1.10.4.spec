#
# $Id: hdf-1.8.spec 950 2012-01-27 17:27:18Z karl $
#
...
%if "%{is_debug}" == "1"
   ...
   %define ncdf_version %{ncdf_pkg_version}-dbg
%else
   %define hdf5_version %{hdf5_pkg_version}
   %define ncdf_version %{ncdf_pkg_version}
%endif
...
for ARCH in "${archA[@]}"; do
  ...
  if [ "$ARCH" = "k1om" ]; then
     LDFLAGS="$LDFLAGS -mmic -Wl,-rpath,$MIC_LIBS -L$MIC_LIBS"
     FC="$FC ${LDFLAGS}"
     CFLAGS="$CFLAGS -mmic"
     FFLAGS="$FFLAGS -mmic"
     FCLAGS="$FCFLAGS -mmic"
...
%if "%{mpi_fam}" != "none"
setenv(       "TACC_PHDF5_DIR",    hdf5_dir)
setenv(       "TACC_PHDF5_DOC",    pathJoin(hdf5_dir,"doc"))
setenv(       "TACC_PHDF5_INC",    pathJoin(hdf5_dir,"include"))
setenv(       "TACC_PHDF5_LIB",    pathJoin(hdf5_dir,"lib"))
setenv(       "TACC_PHDF5_BIN",    pathJoin(hdf5_dir,"bin"))
%endif
