...
#------------------------
  ...
  ./bootstrap.sh --with-python=/opt/apps/gcc5_2/python3/3.7.0/bin/python3.7  --with-python-version=3.7 --with-python-root=/opt/apps/gcc5_2/python3/3.7.0 --prefix=%{PYTHON_INSTALL_DIR} ${CONFIGURE_FLAGS}

  echo "using mpi : /opt/apps/gcc5_2/cray_mpich/7.7.3/bin/mpicxx ;" >> ~/projet-config.jam

  ./b2 -j 24  include="/opt/apps/gcc5_2/python3/3.7.0/include/python3.7m" --prefix=%{PYTHON_INSTALL_DIR} $EXTRA cxxflags="%{TACC_OPT}" cflags="%{TACC_OPT}" linkflags="%{TACC_OPT}" install
