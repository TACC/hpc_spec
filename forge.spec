...
%define DDT_MODULE_DIR  %{OPT}/%{MODULES}/ddt
%define MAP_MODULE_DIR  %{OPT}/%{MODULES}/map
...
#---------------------------
  ...
  rm -rf $RPM_BUILD_ROOT/%{DDT_MODULE_DIR}
  rm -rf $RPM_BUILD_ROOT/%{MAP_MODULE_DIR}
#--------------------------
...
#---------------------------
  ...
  rm -rf  $RPM_BUILD_ROOT/%DDT_MODULE_DIR
  rm -rf  $RPM_BUILD_ROOT/%MAP_MODULE_DIR
  mkdir -p $RPM_BUILD_ROOT/%DDT_MODULE_DIR
  mkdir -p $RPM_BUILD_ROOT/%MAP_MODULE_DIR
  #######################################
  ##### Create TACC Canary Files ########
  #######################################
  touch $RPM_BUILD_ROOT/%{DDT_MODULE_DIR}/.tacc_module_canary
  touch $RPM_BUILD_ROOT/%{MAP_MODULE_DIR}/.tacc_module_canary
  #######################################
  ########### Do Not Remove #############
  #######################################

  cat > $RPM_BUILD_ROOT/%{DDT_MODULE_DIR}/%{version}.lua << 'EOF'
...
setenv("ALLINEA_TOOLS_CONFIG_DIR",pathJoin(home,".forge_%{version}"))
prepend_path("PATH","%{INSTALL_DIR}/bin")
prepend_path("LD_LIBRARY_PATH","%{INSTALL_DIR}/lib")

EOF

  cat > $RPM_BUILD_ROOT/%{MAP_MODULE_DIR}/%{version}.lua << 'EOF'
...
cat > $RPM_BUILD_ROOT/%{DDT_MODULE_DIR}/.version.%{version} << 'EOF'
...
cat > $RPM_BUILD_ROOT/%{MAP_MODULE_DIR}/.version.%{version} << 'EOF'
...
#  %if %{?VISIBLE}
#    %{SPEC_DIR}/checkModuleSyntax $RPM_BUILD_ROOT/%{DDT_MODULE_DIR}/%{MODULE_FILENAME}
#  %endif
#--------------------------
%endif # BUILD_MODULEFILE |
#--------------------------
