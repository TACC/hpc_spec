# Taken from Frontera  remora 1.8.4    may need alterations

# rpmbuild -bb --clean --define 'is_intel19 1' --define 'is_impi 1' --define 'mpiV 19_7' remora-1.8.5.spec 2>&1 | tee remora-1.8.5_i19_r2.log
# rpmbuild -bb --clean --define 'is_intel18 1' --define 'is_impi 1' --define 'mpiV 18_0' remora-1.8.5.spec 2>&1 | tee remora-1.8.5_i18_r1.log

# rpmbuild -bb --clean --define 'is_intel19 1' --define 'is_mvapich2 1' --define 'mpiV 2_3' remora-1.8.5.spec 2>&1 | tee remora-1.8.5_i19_mv2_r1.log
# rpmbuild -bb --clean --define 'is_intel18 1' --define 'is_mvapich2 1' --define 'mpiV 2_3' remora-1.8.5.spec 2>&1 | tee remora-1.8.5_i18_mv2_r1.log

#                    r=/admin/build/admin/rpms/stampede2/RPMS/x86_64
# rpm -hiv --nodeps $r/tacc-remora-intel18-impi18_0-package-1.8.5-1.el7.x86_64.rpm
# rpm -hiv --nodeps $r/tacc-remora-intel18-impi18_0-modulefile-1.8.5-1.el7.x86_64.rpm
...
# 2021-01-27  version 1.8.5
#   All scripts are now BASH.
...
remora ibrun my_MPI_program [arguments]

---------------------------------------
                       # Serial or OpenMP codes
#SBATCH -N 1
#SBATCH -n 1
#SBATCH -A my_project
...
REMORA will create a set of subdirectories (CPU, MEMORY, NUMA, etc.) 
containing text files with the  measured metrics for the run.
In these subdirectories REMORA also creates a set of HTML files
containing time-series google plots of the metrics, for visual analysis. 
In the top directory of the results directory the remora_summary.html
contains links to all the plot files. 
Tar up the directory and transfer it to your own machine to visualize 
the google plots of the data. Just open the remora_summary.html file 
in your browser by double clicking the index, or use an open command.
...
The following environment variables control the behaviour of the tool:
  ...
  - REMORA_VERBOSE - Verbose mode will save all information to
                     a file. Values: 1 or 0. Default is 0 (off).
...
as well as TACC_REMORA_DIR/LIB/INC/DOC: the location of the REMORA 
distribution, libraries, include files, and documents respectively.
