...
%define ispc_version 1.10.0
%define embree_version 3.5.0
%define ospray_version %{version}
%define intel_tbb_path /opt/intel/compilers_and_libraries_2018.2.199/linux/tbb
...
mv ispc-%{ispc_version}-Linux ispc
...
#------------------------
  ...
  cp %{_sourcedir}/ispc/bin/ispc $WORK_INSTALL_DIR/bin

  module load intel/18.0.2 cmake

cd $WORK_DIR
  mkdir embree
  cd embree
  CC=icc CXX=icpc cmake %{_sourcedir}/embree -DCMAKE_INSTALL_PREFIX:PATH=$WORK_INSTALL_DIR -DEMBREE_ISPC_EXECUTABLE:FILE=$WORK_INSTALL_DIR/bin/ispc -DEMBREE_TUTORIALS:BOOL=OFF -DEMBREE_TBB_ROOT:PATH=%{intel_tbb_path}
  make -j4
  make install
