...
export PETSC_CONFIGURE_OPTIONS="\
  --with-x=0 -with-pic \
  --with-np=8 \
  --with-packages-build-dir=${EXTERNAL_PACKAGES_DIR} \
  "
export nopackage="\
...
#
# Elemental, Mumps, Superlu depend on parmetis which depends on metis
...
*i64* ) INDEX_OPTIONS=--with-64-bit-indices ;
	...
	ELEMENTAL_OPTIONS= ; ELEMENTAL_STRING= ;
...
##
%define complexpackages ${ELEMENTAL_STRING} ${hdf5string} ${MUMPS_STRING} scalapack ${SPOOLES_STRING} ${SUITESPARSE_STRING} ${SUPERLU_STRING}
