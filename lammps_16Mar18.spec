...
EOF
  ...
  if [[ $liblist =~ "kim" ]]; then
    ...
    #./configure --prefix="/work/05392/cylu/stampede2/rpmbuild/test/%{INSTALL_SUFFIX}/lib/kim/kim-api" --compiler-suite="GCC"  CXX="mpicxx" CC="mpicc" FC="mpif90"
...
%if %{?BUILD_PACKAGE}
  ...
  #mv src/liblammps_stampede.so          $RPM_BUILD_ROOT/%{INSTALL_DIR}/bin
  #mv src/liblammps_stampede.a           $RPM_BUILD_ROOT/%{INSTALL_DIR}/bin
  ...
  cat > $RPM_BUILD_ROOT/%{MODULE_DIR}/%{version}.lua <<EOF
    local help_message = [[
      ...
      - LIBRARIES
        ...
        LAMMPS libraries liblammps_stampede.a (static), liblammps_stampede.so (dynamic) are kept in TACC_LAMMPS_SRC.
