...
%if "%{comp_fam}" == "intel"
  ...
  export MKL_FLAGS="-I/opt/apps/gcc/${GCC_VERSION}/include/c++/${GCC_VERSION} -I/opt/apps/gcc/${GCC_VERSION}/include/c++/${GCC_VERSION}/x86_64-unknown-linux-gnu -mkl"
# leads to: /opt/apps/gcc/4.9.3/include/c++/4.9.3/ext/atomicity.h(49): error: identifier "__ATOMIC_ACQ_REL" is undefined
  export IFCORE_FLAG="-lifcore"
%endif
%if "%{comp_fam}" == "gcc"
  module load mkl
%endif
...
cmake -VV \
  ...
  \
  -D CMAKE_Fortran_FLAGS:STRING="${MKL_FLAGS} ${IFCORE_FLAG}" \
  -D Trilinos_EXTRA_LINK_FLAGS:STRING="${IFCORE_FLAG}" \
  -D CMAKE_C_FLAGS:STRING="${MKL_FLAGS}" \
  -D CMAKE_CXX_FLAGS:STRING="${MKL_FLAGS}" \
...
( cd %{INSTALL_DIR} && \
  find . -name \*.cmake \
         -exec sed -i -e '/STKDoc_testsConfig.cmake/d' \
                      ...
                      -e '/EXTRA_LD_FLAGS/s?""?"/opt/apps/intel16/python/2.7.12/lib/libpython2.7.so"?' \
                      -e '/SET.*TPL_LIBRARIES/s?""?"/opt/apps/intel16/python/2.7.12/lib/libpython2.7.so"?' \
                      -e '/SET.*TPL_LIBRARIES/s?so"?so;/opt/apps/intel16/python/2.7.12/lib/libpython2.7.so"?' \
                   {} \; \
         -print \
)
...
%changelog
#
* Wed Aug 23 2017 eijkhout <eijkhout@tacc.utexas.edu>
- release 7: adding explicit python lib
* Mon Jul 03 2017 eijkhout <eijkhout@tacc.utexas.edu>
- release 6: careful post-processing for deal
* Wed May 17 2017 eijkhout <eijkhout@tacc.utexas.edu>
- release 5: up to 12.10.1
