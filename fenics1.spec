...
##
## Installation of the first official 1.0 release
## also confusingly referred to as 2019.1.0
## so we call it 2019.1.0r
##
...
#### optional: hdf5
####
export HDF5MODULE=phdf5/1.10.4
...
( \
  ...
  && mkdir -p ${FFC_DIR} \
  && cd ${FFC_DIR} \
  && ${PIP} install fenics-ffc==${FFC_VERSION} \
    --upgrade \
...
( cd mshr/build \
  && cmake \
     ...
     -D BOOST_ROOT:FILEPATH="${TACC_BOOST_DIR}" \
     -D EIGEN3_INCLUDE_DIR:FILEPATH="${EIGEN_INSTALL_DIR}/include/eigen3" \
     -D PETSC_DIR:PATH=${TACC_PETSC_DIR} \
     -D SLEPC_DIR:PATH=${TACC_SLEPC_DIR} \
     -D HDF5_DIR:PATH=${TACC_HDF5_DIR} \
     .. \
  && make install \
) 
...
# /home1/apps/intel18/impi18_0/fenics/2019/python/lib/python3.7/site-packages/pkgconfig
...
prepend_path("PKG_CONFIG_PATH", pathJoin(fenics_dir,"lib64","pkgconfig") )

setenv("FENICS_DIR",             fenics_dir)
setenv("TACC_FENICS_DIR",        fenics_dir)

depends_on("${BOOSTMODULE}")
depends_on("python${PYTHONLEVEL}")
depends_on("${PETSCMODULE}")
depends_on("${SLEPCMODULE}")
depends_on("${HDF5MODULE}")
EOF
...
%changelog
* Wed Jul 03 2019 eijkhout <eijkhout@tacc.utexas.edu>
- release 2: upgrading to petsc 3.11,
    fixing missing HDF5 symbol by leaving hdf5 up to petsc
