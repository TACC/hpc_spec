...
# siliu modified the following session on 01/22/2016
Name: slurm
Version: 15.08.12
Release: 2%{?dist}
...
Packager: Si Liu<siliu@tacc.utexas.edu>
BuildRoot: %{_tmppath}/%{name}-%{version}-%{release}
URL: http://slurm.schedmd.com/

%include rpm-dir.inc
#Si has reorganized and optimized this session
%define Patch1 %{name}-%{version}-patch1.patch
%define Patch2 %{name}-%{version}-patch2.patch
%define Patch3 %{name}-%{version}-patch3.patch
%define Patch4 %{name}-%{version}-patch4.patch
%define Patch5 %{name}-%{version}-patch5.patch
%define Patch6 %{name}-%{version}-patch6.patch
%define Patch9 %{name}-%{version}-patch9.patch
...
#siliu modified the following part on 1/24/2015
%prep
%setup -n %{name}-%{version}
%build

# siliu added the following session on 1/24/2015
# TACC patches
patch  -p0 --verbose < $RPM_SOURCE_DIR/%{Patch1}
patch  -p0 --verbose < $RPM_SOURCE_DIR/%{Patch2}
patch  -p0 --verbose < $RPM_SOURCE_DIR/%{Patch3}
patch  -p0 --verbose < $RPM_SOURCE_DIR/%{Patch4}
patch  -p0 --verbose < $RPM_SOURCE_DIR/%{Patch5}
patch  -p0 --verbose < $RPM_SOURCE_DIR/%{Patch6}
patch  -p0 --verbose < $RPM_SOURCE_DIR/%{Patch9}

# siliu changed the following session on 1/24/2015
%configure --program-prefix=%{?_program_prefix:%{_program_prefix}} --with-mysql_config=/usr/bin/ \
...
%exclude %{_perldir}/Slurm.pm
%exclude %{_perldir}/Slurm/Bitstr.pm
%exclude %{_perldir}/Slurm/Constant.pm
%exclude %{_perldir}/Slurm/Hostlist.pm
%exclude %{_perldir}/Slurm/Stepctx.pm
%exclude %{_perldir}/auto/Slurm/Slurm.so
%exclude %{_perldir}/Slurmdb.pm
%exclude %{_perldir}/auto/Slurmdb/Slurmdb.so
%exclude %{_perldir}/auto/Slurmdb/autosplit.ix
%exclude %{_perlman3dir}/Slurm*

#############################################################################
# siliu: RPM-2  
### siliu made the changed here on 1/24/2015
...
%{_mandir}/man3/slurm_*
#siliu This is a new one in version 14+
...
%endif
#############################################################################
#siliu: RPM-3 OK, Nothing to change.
%if %{slurm_with munge}
%files munge
%defattr(-,root,root)
...
%endif
#############################################################################
# siliu: This is comment out completely!
...
###%{_perlman3dir}/Slurm*
#############################################################################
#siliu: RPM-4 OK, Nothing to change.
...
#############################################################################
#siliu: RPM-5 OK, Nothing to change.
%files -f sql.files sql
%defattr(-,root,root)
%dir %{_libdir}/slurm
#############################################################################
#siliu: RPM-6 modified the folloiwng part on 1/24/2015
...
#############################################################################
#siliu: RPM-7 OK, Nothing to change.
%files torque
%defattr(-,root,root)
%{_bindir}/pbsnodes
%{_bindir}/qalter
...
#############################################################################
#siliu: RPM-8 OK, Nothing to change.
%files sjobexit
%defattr(-,root,root)
%{_bindir}/sjobexitmod
%{_mandir}/man1/sjobexit*
#############################################################################
#siliu: RPM-9 OK, Nothing to change.
...
#siliu: RPM-10 OK, Nothing to change.
%if %{slurm_with pam}
