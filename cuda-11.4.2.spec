...
%define driver_version 470.57.02
...
bash %{SOURCE0} --silent --samples --toolkit --installpath=$RPM_BUILD_ROOT%{INSTALL_DIR}

# fix issue with python shebang not being set to python3
cd $RPM_BUILD_ROOT%{INSTALL_DIR}
find . -path "./nsight-systems-2021.3.2/*.py" | xargs sed -i -e 's/#!\/usr\/bin\/env python/#!\/usr\/libexec\/platform-python/'
