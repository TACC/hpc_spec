...
%define pkg_base_name boost-%{suf}
...
#------------------------
  ...
  cd icu/source
  CXXFLAGS="%{TACC_VEC_OPT}" CFLAGS="%{TACC_VEC_OPT}" ./runConfigureICU  $ICU_MODE --prefix=%{INSTALL_DIR} 
  make -j 28
  make install
  ...
  ./b2 -j 28 --prefix=%{INSTALL_DIR} $EXTRA cxxflags="%{TACC_VEC_OPT}" cflags="%{TACC_VEC_OPT}" linkflags="%{TACC_VEC_OPT}" install
