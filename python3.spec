...
if [ ! -f "%{INSTALL_DIR_COMP}/bin/python3" ]; then
    ...
    ls
    %if "%{comp_fam_name}" == "Intel"
    ./configure --prefix=%{INSTALL_DIR_COMP} CC=icc CXX=icpc LD=xild AR=xiar LIBS='-lpthread -limf -lirc' CFLAGS="-O3 -fp-model strict -fp-model source -ipo -prec-div -prec-sqrt $ISAFLAGS" LDFLAGS="-ipo -Xlinker -export-dynamic" CPPFLAGS="" CPP="icc -E" --with-system-ffi --with-cxx-main=icpc --enable-shared --with-pth --without-gcc --with-libm=-limf --with-threads --with-lto  --with-computed-gotos --with-ensurepip #--enable-optimizations
    %endif
    %if "%{comp_fam_name}" == "GNU"
    ./configure --prefix=%{INSTALL_DIR_COMP} CFLAGS="-flto -ffat-lto-objects -fuse-linker-plugin $ISAFLAGS" LDFLAGS="-flto -ffat-lto-objects -fuse-linker-plugin -rdynamic" --with-system-ffi --enable-shared --with-pth --with-threads --with-lto --with-computed-gotos --with-ensurepip #--enable-optimizations 
    %endif
...
#%{INSTALL_DIR_COMP}/bin/pip3 install supervisor
%{INSTALL_DIR_COMP}/bin/pip3 install paramiko
#%{INSTALL_DIR_COMP}/bin/pip3 install egenix-mx-base
...
#%{INSTALL_DIR_COMP}/bin/pip3 install --no-binary :all: fortran-magic
#%{INSTALL_DIR_COMP}/bin/pip3 install --no-binary :all: MySQL
...
#    %{INSTALL_DIR_COMP}/bin/pip3 install --no-binary :all: h5py
#    #%{INSTALL_DIR_COMP}/bin/pip3 install --no-binary :all: tables
...
#%Module1.0####################################################################
##
## Version file for Python 3 Compiler version %{version}
##
