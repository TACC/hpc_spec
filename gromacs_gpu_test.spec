...
GPU enabled version of mdrun with the suffix of _gpu (you must load cuda/9.0).
...
threads use the -ntomp command line argument. Alternatively, the standard
OMP_NUM_THREADS environment variable can also be used. For example,

  export OMP_NUM_THREADS=8

Given a dual-socket Intel machine with two 8-core CPUs and a Tesla K20 GPU, simply starting the mdrun with the default launch configuration will

  ibrun mdrun_mpi_gpu -pin on -v -s box_md.tpr -deffnm box_md -c box_afterMD.pdb

We recommend to keep -n equal to -N in gpu runs, which means one MPI task per node since there is a single K20 card per node.
