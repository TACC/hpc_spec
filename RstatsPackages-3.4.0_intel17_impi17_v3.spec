...
wget -q -N 'https://cran.r-project.org/src/contrib/Rmpi_0.6-7.tar.gz'
...
#R CMD INSTALL Rmpi_0.6-7.tar.gz --lib=/admin/build/R/x86_64-unknown-linux-gnu-library/3.5 --configure-args="--with-Rmpi-include=${MPICH_HOME}/include64 --with-Rmpi-libpath=${MPICH_HOME}/intel64/lib --with-Rmpi-type=OPENMPI"

# Jail Hack
unset I_MPI_FABRICS
R CMD INSTALL Rmpi_0.6-7.tar.gz --no-tests --configure-args="--with-Rmpi-include=${MPICH_HOME}/include64 --with-Rmpi-libpath=${MPICH_HOME}/intel64/lib --with-Rmpi-type=OPENMPI"
...
sleep 5
tar xvfz gdal-2.3.1.tar.gz
GDAL_HOME=%{INSTALL_DIR}/gdal-2.3.1
export GDAL_HOME
mkdir ${GDAL_HOME}
cd gdal-2.3.1
...
wget -q -N http://download.osgeo.org/proj/proj-5.1.0.tar.gz
sleep 5
tar xvfz proj-5.1.0.tar.gz
PROJ_HOME=%{INSTALL_DIR}/proj-5.1.0
export PROJ_HOME
mkdir ${PROJ_HOME}
cd proj-5.1.0
...
wget -q -N https://cran.r-project.org/src/contrib/sp_1.3-1.tar.gz
R CMD INSTALL sp_1.3-1.tar.gz
...
wget -q -N https://cran.r-project.org/src/contrib/Archive/rgdal/rgdal_1.3-3.tar.gz
export LD_LIBRARY_PATH=${GDAL_HOME}/lib:${PROJ_HOME}/lib:${LD_LIBRARY_PATH}
R CMD INSTALL --configure-args="--with-gdal-config=${GDAL_HOME}/bin/gdal-config --with-proj-include=${PROJ_HOME}/include --with-proj-lib=${PROJ_HOME}/lib" rgdal_1.3-3.tar.gz
...
wget -q -N http://ufpr.dl.sourceforge.net/project/mcmc-jags/JAGS/4.x/Source/JAGS-4.3.0.tar.gz 
sleep 10 
tar xvfz JAGS-4.3.0.tar.gz
JAGS_HOME=%{INSTALL_DIR}/jags-4.3.0
export JAGS_HOME
mkdir ${JAGS_HOME}
cd JAGS-4.3.0 
...
wget -q -N https://cran.r-project.org/src/contrib/Archive/coda/coda_0.18-1.tar.gz
sleep 10
R CMD INSTALL coda_0.18-1.tar.gz

wget -q -N http://cran.r-project.org/src/contrib/Archive/rjags/rjags_4-5.tar.gz
sleep 10
