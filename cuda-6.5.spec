...
%if !0%{?rhel}
Source0:	http://developer.download.nvidia.com/compute/cuda/6.5/rel/installers/cuda_%{version}.%{release}.run
%endif
Source1:	http://developer.download.nvidia.com/compute/cuda/6.5/rel/installers/cuda_%{version}.%{release}_linux_64.run
BuildRoot:	%(mktemp -ud %{_tmppath}/%{name}-%{version}-%{release}-XXXXXX)

ExclusiveArch:  		x86_64
Provides:			cuda-6.5 = %{version}-%{release}
AutoReqProv: 			no
...
Group:		Development/Libraries
Requires:	cuda-6.5 = %{version}-%{release}
AutoReqProv: 	no
...
#   -toolkitpath=<PATH> : Specify path for CUDA location (default: /usr/local/cuda-6.5)
#   -samples            : Install CUDA 6.0 Samples
#   -samplespath=<PATH> : Specify path for Samples location (default: /usr/local/cuda-6.5/samples)
...
rm $RPM_BUILD_ROOT%{INSTALL_DIR}/bin/uninstall_cuda_6.5.pl
...
%{INSTALL_DIR}/bin/cuda-install-samples-6.5.sh
