...
Source0: petsc-%{versionpatch}.tar.gz
...
if [ "${ext}" = "tau" ] ; then
  ...
  RPM_BUILD_ROOT=tmpfs PETSC_DIR=`pwd` ./configure \
    F77=$F77 CFLAGS=$CFLAGS CXXFLAGS=$CXXFLAGS FFLAGS=$FFLAGS \
...
PETSC_DIR=`pwd` PETSC_ARCH=${architecture} make MAKE_NP=48 V=1
##
##
