...
%define bazel_minor_version       4
%define bazel_micro_version       2
...
%if "%{python_major_version}" == "2"
  ...
  %define pip_name          pip
  %define python_pkg_date   March 8, 2017
%else
  %define python_pkg_name   python3
  %define pip_name          pip3
  %define python_pkg_date   May 02, 2016 
%endif
...
%if "%{build_type}" == "gpu"
ml git cuda/%{cuda_pkg_version} cudnn/%{cudnn_pkg_version} binutils/2.25 %{python_pkg_name}/%{python_pkg_version}
%endif

%if "%{build_type}" == "cpu"
#ml git binutils/2.25 %{python_pkg_name}/%{python_pkg_version}
...
%{pip_name} install pip   --ignore-installed --install-option="--prefix=%{_builddir}/tensorflow_pip"
%{pip_name} install wheel --ignore-installed --install-option="--prefix=%{_builddir}/tensorflow_pip"
%{pip_name} --version
%{pip_name} list
...
# Now, use earlier version of bazel to build newer version of bazel
cd ${tensorflow}
##git clone https://github.com/bazelbuild/bazel.git
cp -r %{_sourcedir}/bazel .
cd %{bazel_base_name}
...
%if "%{build_type}" == "gpu"
${tensorflow}/%{bazel_base_name}/bazel-bin/src/bazel build -c opt --copt=-mavx --linkopt '-lrt' --genrule_strategy=standalone --spawn_strategy=standalone --config=cuda -s --verbose_failures //tensorflow/tools/pip_package:build_pip_package
%endif
%if "%{build_type}" == "cpu"
${tensorflow}/%{bazel_base_name}/bazel-bin/src/bazel build -c opt --copt=-march=knl --linkopt '-lrt' --genrule_strategy=standalone --spawn_strategy=standalone               -s --verbose_failures //tensorflow/tools/pip_package:build_pip_package
%endif
...
%{pip_name} install --prefix=%{INSTALL_DIR} --no-binary :all: --install-option="--prefix=%{INSTALL_DIR}" ${tensorflow}/%{pkg_base_name}/tensorflow_pkg/tensorflow-%{pkg_version}-cp%{python_squ_version}-cp%{python_squ_version}m-linux_x86_64.whl
...
%define python_impi_inherit   %( if [ ! -f "%{MODULE_PREFIX}/%{comp_fam_ver}/impi17_0/modulefiles/%{python_pkg_name}/%{python_pkg_version}.lua" ]; then echo "%{MODULE_PREFIX}/%{comp_fam_ver}/impi17_0/modulefiles/%{python_pkg_name}/%{python_pkg_version}.lua"; else echo ""; fi )

echo "%{MODULE_PREFIX}/%{comp_fam_ver}/modulefiles/%{python_pkg_name}/%{python_pkg_version}.lua"
echo "python_inherit     '%{python_inherit}'"
echo "python_impi_inherit '%{python_impi_inherit}'"
...
if ! grep -qs 'prepend_path{"MODULEPATH", "%{MODULE_PREFIX}/%{comp_fam_ver}/%{python_fam_ver}/modulefiles", priority=10}' %{MODULE_PREFIX}/%{comp_fam_ver}/impi17_0/modulefiles/%{python_pkg_name}/%{python_pkg_version}.lua; then
  #echo "Appending impi MP %{MODULE_PREFIX}/%{comp_fam_ver}/impi17_0/modulefiles/%{python_pkg_name}/%{python_pkg_version}.lua"
  echo 'prepend_path{"MODULEPATH", "%{MODULE_PREFIX}/%{comp_fam_ver}/%{python_fam_ver}/modulefiles", priority=10}' >> %{MODULE_PREFIX}/%{comp_fam_ver}/impi17_0/modulefiles/%{python_pkg_name}/%{python_pkg_version}.lua
fi
%preun %{MODULEFILE}
#set -x
if grep -qs 'prepend_path{"MODULEPATH", "%{MODULE_PREFIX}/%{comp_fam_ver}/%{python_fam_ver}/modulefiles", priority=10}' %{MODULE_PREFIX}/%{comp_fam_ver}/modulefiles/%{python_pkg_name}/%{python_pkg_version}.lua; then
...
if grep -qs 'prepend_path{"MODULEPATH", "%{MODULE_PREFIX}/%{comp_fam_ver}/%{python_fam_ver}/modulefiles", priority=10}' %{MODULE_PREFIX}/%{comp_fam_ver}/impi17_0/modulefiles/%{python_pkg_name}/%{python_pkg_version}.lua; then
  #echo "Remove impi MP"
  sed -i '\:prepend_path{"MODULEPATH", "%{MODULE_PREFIX}/%{comp_fam_ver}/%{python_fam_ver}/modulefiles", priority=10}:d' %{MODULE_PREFIX}/%{comp_fam_ver}/impi17_0/modulefiles/%{python_pkg_name}/%{python_pkg_version}.lua
fi
%preun %{PACKAGE}
