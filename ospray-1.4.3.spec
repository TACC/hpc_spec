...
URL:       http://ospray.org
Packager:  TACC - jbarbosa@tacc.utexas.edu (pnav on Hikari)
...
%define intel_tbb_path /opt/apps/intel/16.0.1.150/compilers_and_libraries_2016.1.150/linux/tbb
...
#------------------------
  ...
  module load intel/16.0.1 impi
  # build EMBREE (if not already)
    cd $WORK_DIR
    rm -rf embree
    mkdir embree
    cd embree
    CC=icc CXX=icpc cmake %{_sourcedir}/embree -DCMAKE_INSTALL_PREFIX:PATH=$WORK_INSTALL_DIR -DEMBREE_ISPC_EXECUTABLE:FILE=$WORK_INSTALL_DIR/bin/ispc -DEMBREE_MAX_ISA=AVX2 -DEMBREE_TUTORIALS:BOOL=OFF -DEMBREE_TBB_ROOT:PATH=%{intel_tbb_path}
    make -j4
    make install
    ...
    mkdir ospray
    cd ospray
    CC=icc CXX=icpc cmake %{_sourcedir}/ospray -DCMAKE_INSTALL_PREFIX:PATH=$WORK_INSTALL_DIR -DEMBREE_ISPC_EXECUTABLE:FILE=$WORK_INSTALL_DIR/bin/ispc -Dembree_DIR:PATH=${WORK_INSTALL_DIR}/lib64/cmake/embree-2.16.5 -DOPENGL_INCLUDE_DIR:PATH=/opt/apps/swr/17.3.4/include -DOPENGL_gl_LIBRARY:FILEPATH=/opt/apps/swr/17.3.4/lib/libGL.so -DOSPRAY_APPS_EXAMPLEVIEWER:BOOL=OFF -DOSPRAY_APPS_BENCHMARK:BOOL=OFF -DOSPRAY_APPS_GLUTVIEWER:BOOL=OFF -DOSPRAY_APPS_PARAVIEW_TFN_CVT:BOOL=OFF -DOSPRAY_APPS_QTVIEWER:BOOL=OFF -DOSPRAY_APPS_VOLUMEVIEWER:BOOL=OFF -DOSPRAY_MODULE_MPI:BOOL=ON -DOSPRAY_APPS_UTILITIES:BOOL=OFF -DOSPRAY_SG_VTK:BOOL=ON
    make -j4
    make install
