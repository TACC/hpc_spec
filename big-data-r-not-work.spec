...
Source:     rmr2_3.3.1.tar.gz
Source1:    rJava_0.9-6.tar.gz
Source2:    rhdfs_1.0.8.tar.gz
Source3:    scala-2.10.4.tgz
Source4:    SparkR-pkg-master.zip
Source5:    plyrmr_0.6.0.tar.gz
Source6:    protobuf-2.5.0.tar.bz2
Source7:    Rhipe_0.75.0_cdh5mr2.tar.gz
...
export SRC_DIR="/admin/build/rpms/SOURCES"    #${WD}/src
#mkdir -p ${SRC_DIR}
echo ${SRC_DIR}
cd ${SRC_DIR}

## set r-library to store R packages
rm -rf $RPM_BUILD_ROOT%{INSTALL_DIR}/r-library
mkdir -p $RPM_BUILD_ROOT/%{INSTALL_DIR}/r-library
## add a library path for R to install, R CMD INSTALL to
export R_LIBS=$RPM_BUILD_ROOT/%{INSTALL_DIR}/r-library
###########################################################
...
SCALA_HOME=$RPM_BUILD_ROOT/%{INSTALL_DIR}/scala-2.10.4
export SCALA_HOME
rm -rf $RPM_BUILD_ROOT/%{INSTALL_DIR}/scala-2.10.4
cp -r ${SRC_DIR}/scala-2.10.4 $RPM_BUILD_ROOT/%{INSTALL_DIR}

cd ${SRC_DIR}
rm -r scala-2.10.4

export PATH=$RPM_BUILD_ROOT/%{INSTALL_DIR}/scala-2.10.4/bin:$PATH
...
PROTOBUF_HOME=$RPM_BUILD_ROOT/%{INSTALL_DIR}/protobuf-2.5.0
...
#echo "LD_LIBRARY_PATH=%{INSTALL_DIR}/protobuf-2.5.0/lib" >> ${TACC_R_DIR}/lib64/R/etc/Renviron
...
local pkg_config_path = "%{INSTALL_DIR}/protobuf-2.5.0/lib/pkgconfig"
