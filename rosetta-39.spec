...
#------------------------
  ...
  rm -rf %{INSTALL_DIR}
  rm -rf /var/tmp/%{pkg_name}-%{pkg_version}-buildroot
...
sed -i 's/static_assert(std::is_signed<T>() == std::is_signed<decltype(x)>(), "")/static_assert(std::is_signed<T>::value == std::is_signed<decltype(x)>::value, "")/g' external/include/json.hpp
...
./scons.py -c mode=${MODE} extras=${EXTRAS} cxx=${COMPILER}
...
./scons.py -j24 mode=${MODE} extras=${EXTRAS} cxx=${COMPILER} bin 
  ...
  cd $RPM_BUILD_ROOT
  ls -al
  cp -r %{INSTALL_DIR}/rosetta_src_%{build_version}_bundle/main/source/doc   $RPM_BUILD_ROOT/%{INSTALL_DIR}
  cp -r %{INSTALL_DIR}/rosetta_src_%{build_version}_bundle/main/source/bin   $RPM_BUILD_ROOT/%{INSTALL_DIR}
  cp -r %{INSTALL_DIR}/rosetta_src_%{build_version}_bundle/main/source/build $RPM_BUILD_ROOT/%{INSTALL_DIR}
  cp -r %{INSTALL_DIR}/rosetta_src_%{build_version}_bundle/main/database     $RPM_BUILD_ROOT/%{INSTALL_DIR}
